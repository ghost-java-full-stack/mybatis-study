 ...
 * java.sql.BatchUpdateException that is the root cause, as well as the results from any
 * prior nested batch that executed successfully.
 *
 * @author Jeff Butler
 */
public class BatchExecutorException extends ExecutorException {
     ...
     * exception contains an array of row counts that can be used to determine exactly
     * which statement of the executor caused the failure (or failures).
     ...
     * @return the previously successful executor results (may be an empty list if no
     * executor has executed successfully)
     */
