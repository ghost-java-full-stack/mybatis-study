...
class CursorSimpleTest {
    ...
    static void setUp() throws Exception {
        ...
        try (Reader reader = Resources
            .getResourceAsReader("org/apache/ibatis/submitted/cursor_simple/mybatis-config.xml")) {
    ...
    void testCursorClosedOnSessionClose() {
        ...
        // The cursor was not fully consumed, but it should be close since we closed the
        // session
        Assertions.assertFalse(usersCursor.isOpen());
        Assertions.assertFalse(usersCursor.isConsumed());
    }
    ...
    void testCursorWithBadRowBound() {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            // Trying to start at offset 10 (which does not exist, since there is only 4
            // items)
    ...
    void shouldThrowIllegalStateExceptionUsingIteratorOnSessionClosed() {
        ...
        try {
            ...
            Assertions
                .fail("Should throws the IllegalStateException when call the iterator method after session is closed.");
        }
        catch (IllegalStateException e) {
